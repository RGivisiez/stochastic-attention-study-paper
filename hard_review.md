...
**W3 — The experimental baselines are far too weak.** ⚠️ **PARTIALLY ADDRESSED.**
...
> **Author response (partial):** The VAE baseline now runs on all MNIST digits (1, 3, 8) and on the protein sequence domain, using the same two-phase training protocol (AE warmup + KL annealing). The VAE consistently emerges as the strongest non-Langevin baseline across all domains, confirming the ranking is not cherry-picked. On proteins (PF00076 RRM family), the VAE achieves novelty 0.621 and seq identity 0.532, but SA dominates on amino acid composition fidelity (KL 0.107 vs 0.416). A DDPM/flow-matching baseline remains to be added.
