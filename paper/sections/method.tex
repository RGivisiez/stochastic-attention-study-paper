...
Because $\mathbf{X}$ is given as data, there are no learned parameters: no training loop, no contrastive divergence, and no score-matching objective. The confining bound~\eqref{eq:energy-lower-bound} keeps iterates concentrated near $\mathrm{conv}\{\mathbf{m}_1,\dots,\mathbf{m}_K\}$ with excursions controlled by $\sqrt{2\alpha/\beta}$; Section~\ref{sec:phase-transition} derives a $\beta$-selection rule from the entropy of the attention distribution.
...
For fixed $\boldsymbol{\xi}\in\mathbb{R}^N$, let $e_k = \mathbf{m}_k^{\top}\boldsymbol{\xi}$ and $\mathbf{p}=\operatorname{softmax}(\beta\mathbf{e})$. Then
...
When the $K$ memories are drawn uniformly on the unit sphere $\mathbb{S}^{N-1}$, the similarities $e_k$ are approximately iid with $\mathrm{Var}(e_k)=1/N$. The softmax logits $\beta e_k$ have standard deviation $\beta/\sqrt{N}$; the transition from near-uniform to concentrated attention occurs when this standard deviation reaches order unity, giving
...
\mathrm{SNR}^{*} \;=\; \sqrt{\frac{\alpha\,\beta^{*}}{2N}} \;\sim\; \sqrt{\frac{\alpha}{2\sqrt{N}}}\,.
\end{equation}
At $N{=}64$ and $\alpha{=}0.01$ this evaluates to $\mathrm{SNR}^{*}=\sqrt{0.01/16}=0.025$, exactly matching the empirical transition observed in Section~5. The threshold is not a universal constant: it scales as $N^{-1/4}$ and depends on the pattern geometry through $\mathrm{Var}(e_k)$. For structured data the effective variance may differ substantially from $1/N$, but Eq.~\eqref{eq:inflection} provides a fully data-dependent criterion that applies in any domain: evaluate $d^{2}H/d\beta^{2}$ over a sweep of $\beta$ values and locate the zero crossing.